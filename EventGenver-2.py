"""EventGen.py v1.4.0 - Financial Event Generation Engine
Combines VtoT(3), Interpret, TextEXT, FinContext, and optionally FinBERT outputs.
Multi-evidence rules, human-in-the-loop, no raw audio access.

IMPORTANT: Events require 2+ independent signal sources to emit.
Nearby same-type events are merged unless there's a context break."""

import json, sys
from typing import Dict, List, Optional, Set
from datetime import datetime, timezone

VERSION, ARTIFACT_TYPE = "1.4.0", "financial_event_generation"

# Event merging configuration
MERGE_WINDOW_SECONDS = 30  # Merge same-type events within 30 seconds
CONTEXT_BREAK_GAP = 60     # Gap > 60s is a context break
...
class EventGenerator:
    def __init__(self, vtot: Dict, signals: Dict, markers: Dict, context: Dict, finbert: Optional[Dict] = None):
        self.vtot = vtot
        ...
        self.finbert = finbert.get('sentences', []) if finbert else []
        ...
        self.finbert_by_idx = {s['sentence_index']: s for s in self.finbert}
    
    def _get_uncertainty(self, idx: int) -> float:
        """Get FinBERT uncertainty score (advisory only)."""
        return self.finbert_by_idx.get(idx, {}).get('uncertainty_score', 0)
    
    def _has_risk_language(self, idx: int) -> bool:
        """Check FinBERT risk language flag (advisory only)."""
        return self.finbert_by_idx.get(idx, {}).get('risk_language_detected', False)
    ...
    def _rule_affordability_signal(self):
        ...
        for idx in range(len(self.sentences)):
            ...
            if afford_event and current_band == afford_event.get('financial_context', {}).get('amount_band'):
                ...
                # Add FinBERT evidence if available (advisory only)
                uncertainty = self._get_uncertainty(idx)
                if uncertainty > 0:
                    evidence.append({"source": "FinBERT", "uncertainty_score": uncertainty, "sentence": idx, "advisory": True})
    ...
    def _count_signal_sources(self, evidence: List[Dict]) -> int:
        """Count unique independent signal sources in evidence."""
        sources = set()
        for e in evidence:
    ...
    def _should_merge(self, prev: Dict, curr: Dict) -> bool:
        """Determine if two events should be merged (same type, within window, no context break)."""
        if prev['event_type'] != curr['event_type']:
            return False
        time_gap = curr['timestamp']['start'] - prev['timestamp']['end']
        if time_gap > MERGE_WINDOW_SECONDS:
            return False
        # Context break: amount band change
        if prev['financial_context']['amount_band'] != curr['financial_context']['amount_band']:
            return False
        return True
    
    def _merge_events(self, events: List[Dict]) -> List[Dict]:
        """Merge nearby same-type events unless context break."""
        if not events:
            return []
        
        # Sort by start time
        sorted_events = sorted(events, key=lambda e: e['timestamp']['start'])
        merged = []
        ...
        for event in sorted_events:
            ...
            if self._should_merge(merged[-1], event):
                # Merge: extend end time, combine evidence, update explanation
                merged[-1]['timestamp']['end'] = max(merged[-1]['timestamp']['end'], event['timestamp']['end'])
                # Combine evidence (dedupe by source+type)
                existing_keys = {(e.get('source',''), e.get('indicator', e.get('marker',''))) 
                                for e in merged[-1]['evidence']}
                for e in event['evidence']:
                    key = (e.get('source',''), e.get('indicator', e.get('marker','')))
                    if key not in existing_keys:
                        merged[-1]['evidence'].append(e)
                        existing_keys.add(key)
                # Update explanation to note merge
                merged[-1]['explanation'] += f" [merged with event at sentence {event['sentence_index']}]"
            else:
                merged.append(event)
    ...
    def generate(self) -> Dict:
        ...
        # Two-signal rule: filter events that don't have 2+ independent sources
        # (except PII which can stand alone as required by regulation)
        validated = []
        for e in self.events:
            source_count = self._count_signal_sources(e['evidence'])
            if source_count >= 2 or e['event_type'] == 'pii_sensitive_call':
                e['signal_source_count'] = source_count
                validated.append(e)
        
        # Dedupe by (type, sentence_index)
        seen: Set[tuple] = set()
        unique = [e for e in validated if (key := (e['event_type'], e['sentence_index'])) not in seen and not seen.add(key)]
        
        # Merge nearby same-type events
        merged = self._merge_events(unique)
        ...
        return {
            ...
            "summary": {"total_events": len(merged), 
                       "events_by_type": {t: sum(1 for e in merged if e['event_type'] == t) for t in EVENT_TYPES}}
        }
...
def main():
    if len(sys.argv) < 5:
        print(json.dumps({"error": "usage: EventGen.py vtot.json signals.json markers.json context.json [finbert.json]"}))
        sys.exit(1)
    finbert = load_json(sys.argv[5]) if len(sys.argv) > 5 else None
    engine = EventGenerator(load_json(sys.argv[1]), load_json(sys.argv[2]), load_json(sys.argv[3]), load_json(sys.argv[4]), finbert)
