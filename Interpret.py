"""Interpret.py v1.1.0 - Behavioral Change Indicator Transformer
Detects deviations from baseline in speech metrics. Text-only, no inference."""

import json, sys
from typing import Dict, List, Optional

VERSION, ARTIFACT_TYPE = "1.1.0", "behavioral_signal_transform"
...
def transform(data: Dict) -> Dict:
    ...
    for idx, s in enumerate(sents):
        ...
        if not low_q and not any(i['grade'] > EXTREME_THRESHOLD for i in indicators):
            if m['speed'] > 0: hist['speed'].append(m['speed'])
            hist['pause'].append(m['pause'])
            if m['conf'] > 0: hist['conf'].append(m['conf'])
            if m['wc'] > 0: hist['wc'].append(m['wc'])
