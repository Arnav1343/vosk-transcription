"""TextEXT.py v1.2.0 - Text Marker Extraction
Extracts text markers (financial, product, commitment, regulatory, PII) using pattern matching."""

import json, re, sys
from typing import Dict, List, Set

VERSION, ARTIFACT_TYPE = "1.2.0", "text_marker_extraction"
...
CURRENCY = re.compile(r'\$\s*\d+(?:,\d{3})*(?:\.\d{2})?|\d+(?:,\d{3})*(?:\.\d{2})?\s*(?:dollars?|usd)\b', re.I)
...
PII = {"phone": re.compile(r'\b(?:\d{3}[-.\s]?\d{3}[-.\s]?\d{4}|\(\d{3}\)\s*\d{3}[-.\s]?\d{4}|\d{10,11}|[a-z]-?\d{2}-?\d{3}-?\d{4})\b', re.I),
       "account": re.compile(r'\b\d{4,}(?:[-\s]\d{2,})+\b|\b\d{8,16}\b'),
       "numeric_id": re.compile(r'\b\d{5}(?:-\d{4})?\b'), "sequence": re.compile(r'\b\d{5,}\b')}
